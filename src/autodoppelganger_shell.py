...
class AutoDoppelGANgerShell:
    def __init__(self):
        ...
        self.model_vae = BetaVAETrainer(
            3, 128, [32, 64, 128, 256, 512], device, "logs", "models/checkpoints/"
        )
    ...
    def help(self):
        ...
        print(
            "gensam - Generate Samples (Usage: gensam <type> <num_samples> <num_rows> <num_columns>)"
        )
        print(
            "ldwghtsgan - Load model weights (Usage: ldwghtsgan <path to discriminator weights> <path to generator weights>)"
        )
        ...
        print(
            "trainbvae - Start training the Beta-VAE model (Usage: trainbvae <path to json file>)"
        )
    ...
    def train_gan(self, json_file):
        ...
        try:
            ...
            self.model.train(
                self.dataset,
                training_setup["num_epochs"],
                training_setup["batch_size"],
                training_setup["learning_rate"],
                training_setup["beta1"],
                training_setup["beta2"],
                training_setup["time_limit"],
            )
            if training_setup["save_weights"]:
                discriminator = (
                    "models/checkpoints/"
                    + training_setup["discriminator_weights_filename"]
                    + ".pth"
                )
                generator = (
                    "models/checkpoints/" + training_setup["generator_weights_filename"] + ".pth"
                )
    ...
    def train_bvae(self, json_file):
        ...
        try:
            ...
            self.model_vae.train(
                ...
                training_setup["beta"],
                training_setup["learning_rate"],
                training_setup["weights_filename"],
            )
        except FileNotFoundError as e:
            print(f"Configuration file not found: {e}")
        except json.JSONDecodeError as e:
    ...
    def generate_samples(self, type, num_samples, num_rows, num_cols):
        ...
        try:
            ...
            print(
                "Pass correct arguments: <type(gan/bvae)> <num_samples> <num_rows> <num_columns>"
            )
                ...
                if user_input:
                    ...
                    if parts[0] == "help":
                        ...
                        try:
                            ...
                            print(
                                "Usage: ldwghtsgan <path to discriminator weights> <path to generator weights>"
                            )
