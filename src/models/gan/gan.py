...
class GAN:
    ...
    def train(
        self, dataset, num_epochs, batch_size, learning_rate, beta1=0.5, beta2=0.999, time_limit=1
    ):
            ...
            for batch_idx, (real, _) in enumerate(self.loaded_data):
                ...
                if batch_idx % 100 == 0:
                    self.print_training_stats(
                        num_epochs, epoch, batch_idx, len(self.loaded_data), loss_gen, loss_dsc
                    )
                    # generated_images = self.generate_samples(10, fixed_noise)
                    # self.plot_grid(generated_images, 5, 2, index=batch_idx)
                    self.tensor_board_grid(
                        fixed_noise, self.writer_real, self.writer_fake, real, step
                    )
                    step += 1
            epoch += 1
    ...
    def setup_optimizers(self, learning_rate, beta1, beta2):
        self.opt_gen = optim.Adam(
            self.generator.parameters(), lr=learning_rate, betas=(beta1, beta2)
        )
        self.opt_dsc = optim.Adam(
            self.discriminator.parameters(), lr=learning_rate, betas=(beta1, beta2)
        )
